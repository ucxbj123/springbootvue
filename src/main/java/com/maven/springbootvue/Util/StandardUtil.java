...
public class StandardUtil {
    ...
    //1、校验值的类型类型--检验失败原因：参考值不是数字类型
    public static Boolean checkType(Teststandard teststandard){
        Boolean res = false;
    ...
    //2、根据起始位置和长度进行校验
    public static StandardResult checkValue(Teststandard teststandard,String code, int len){
        //存储校验结果
        ...
        if (startindex > len || endindex > len){//如果截取的位置已经超过条码长度则不需要往后操作
            ...
            if(!teststandard.getLogicvalue().equals("")){//若逻辑值不为空则进行后续第二个参考值和检验条件的校验
    ...
    public static StandardResult checkcode(String type, String code, String CheckValue, String CheckCondition){
        //存储校验结果
        StandardResult result = new StandardResult();
        //字符串进行比较的字符
